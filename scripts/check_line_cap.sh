#!/usr/bin/env bash
#
# check_line_cap.sh — ratcheted 500-line file-size cap enforcement.
#
# Ported from trusty-tools' gate (its issue #610), adapted to C# (*.cs). navigaT-SQL
# is a sidekick to trusty-tools and mirrors its development workflow.
#
# Why: a file-size cap that lives only in docs loses under feature pressure —
#   files silently grow past it. This turns the cap into a CI/pre-commit gate
#   whose grandfather allowlist can only SHRINK: no new oversized file may be
#   added, and existing oversized files may never grow.
#
# What: scans every tracked `.cs` file (`git ls-files '*.cs'`) and enforces:
...
#
# Test: the logic is pure line counting against the committed allowlist; verify
#   manually (clean tree exits 0; a 501-line probe file fails; appending past an
#   allowlisted budget fails). No unit-test harness.
#
...
# ---------------------------------------------------------------------------
# Build a current snapshot: "<lines>\t<path>" for every tracked .cs file that
...
if [ "$MODE" = "update" ]; then
  ...
  {
    echo "# .line-cap-allowlist.tsv — grandfathered files over the ${CAP}-line cap."
...
# snapshot rows -> "C\tlines\tpath". This survives an empty allowlist file.
{
  ...
  END {
    ...
    for (p in have) if (!(p in seen)) {
      printf "WARN: allowlisted %s no longer exists as a tracked .cs file. Remove it from .line-cap-allowlist.tsv.\n", p
    }
